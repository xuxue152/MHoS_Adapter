...
class BaseDataset(Dataset, metaclass=ABCMeta):
    def __init__(self,
                 ann_file,  # .txt or .json   split the train and test data
                 pipeline,  # transforms
                 repeat=1,
                 data_prefix=None,  # source path of dataset
                 test_mode=False,
                 multi_class=False,
                 num_classes=None,
                 start_index=1,
        ...
        self.video_infos = self.load_annotations()  # [[abspath, label, format], [abspath, label, format], ...]
...
class VideoDataset(BaseDataset):
    def __init__(self, ann_file, pipeline, labels_file, start_index=0,k=None, **kwargs):
        super().__init__(ann_file, pipeline, start_index=start_index, k=k, **kwargs,)
    ...
    def load_annotations(self):
        ...
        if self.k is not None:
            ...
            for item in video_infos:
                label = item['label']
                video_infos_by_class[label].append(item)
            for _, samples in video_infos_by_class.items():
                num_samples = len(samples)
                np.random.seed(1)  # select same samples in each experiment
                select_id = np.random.randint(0, num_samples, num_shot)
                # select_id = [44,47,48,64,65,67,9,21,36,37,12,15]
                # print(select_id)
                samples = [samples[i] for i in select_id]
                few_shot_infos.extend(samples)
            video_infos = few_shot_infos

        return video_infos  # [dict{abspath, label, format}, dict{abspath, label, format}, ...]
...
def build_dataloader(logger, config):
    ...
    train_data = VideoDataset(ann_file=config.DATA.TRAIN_FILE, data_prefix=config.DATA.ROOT,
                              labels_file=config.DATA.LABEL_LIST, pipeline=train_pipeline, k=config.DATA.K)
    ...
    # zy
    val_data = VideoDataset(ann_file=config.DATA.VAL_FILE, data_prefix=config.DATA.VAL_ROOT, labels_file=config.DATA.LABEL_LIST, pipeline=val_pipeline)
